...
let spec =
  ...
  empty
  +> flag "-n" (optional string) ~doc: "The name of the jornal (makes a new one if it doesn't exist)" (* anonomus entry *)
  +> flag "-m" (required string) ~doc: "the message of the entry"
